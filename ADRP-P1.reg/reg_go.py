...
checkpointer = ModelCheckpoint(filepath='agg_attn.autosave.model.h5', verbose=1, save_weights_only=False, save_best_only=True)
csv_logger = CSVLogger('agg_attn.training.log')
...
plt.savefig('agg_attn.mae.png', bbox_inches='tight')
plt.savefig('agg_attn.mae.pdf', bbox_inches='tight')
...
plt.savefig('agg_attn.loss.png', bbox_inches='tight')
plt.savefig('agg_attn.loss.pdf', bbox_inches='tight')
...
with open("agg_attn.model.json", "w") as json_file:
        json_file.write(model_json)

# serialize model to YAML                                                                                                                     
model_yaml = model.to_yaml()
with open("agg_attn.model.yaml", "w") as yaml_file:
        yaml_file.write(model_yaml)


# serialize weights to HDF5                                                                                                                   
model.save_weights("agg_attn.model.h5")
print("Saved model to disk")

exit()

# load json and create model                                                                                                                  
json_file = open('agg_attn.model.json', 'r')
...
yaml_file = open('agg_attn.model.yaml', 'r')
...
loaded_model_json.load_weights("agg_attn.model.h5")
print("Loaded json model from disk")

# evaluate json loaded model on test data                                                                                                     
loaded_model_json.compile(loss='binary_crossentropy', optimizer='SGD', metrics=['mean_absolute_error'])
...
# load weights into new model                                                                                                                 
loaded_model_yaml.load_weights("agg_attn.model.h5")
print("Loaded yaml model from disk")

# evaluate loaded model on test data                                                                                                          
loaded_model_yaml.compile(loss='binary_crossentropy', optimizer='SGD', metrics=['mean_absolute_error'])
